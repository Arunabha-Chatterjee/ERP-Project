...
import useCustomer from "../../../hook/useCustomer/useCustomer";
import Message from "../../common/Message";
import useUpdateCustomer from "../../../hook/useCustomer/useUpdateCustomer";
...
const UpdateCustomerForm = () => {
    ...
    const onSubmit = async (data)=>{
        try{
            ...
            navigate(`/customers/${customer.customerId}/description`);
            toast.success(response);
        }catch(error){
            toast.error(error.response.data || "Error in update customer");
        }
    }
    ...
    return (
        <div className='w-screen h-screen font-mono bg-black flex justify-center items-center text-white '>
            <div className="w-[34%] h-auto border border-gray-700 ">
                {isCustomerFetching ? (<Message message={"Loading..."} />
                ) : customerError ? (<Message message={customerError} />
                ) :
                    <form onSubmit={handleSubmit(onSubmit)}>
                        <div
                            className='w-full py-1.5 pl-2 pr-1 bg-black text-white flex justify-between border-b border-gray-700'>
                            <span>Update Customer - {customerId}</span>
                            <button className='text-lg px-3 h-full
                    hover:bg-red-600 cursor-pointer'
                    type="button"
                                onClick={() => navigate(`/customers/${customer.customerId}/description`)}>X
                            </button>
                        </div>
                        ...
                        <div className='grid grid-cols-2 gap-x-1 gap-y-2 py-4 px-3 text-white'>
                            ...
                            <button
                            type="submit" 
                            className='border-1 border-gray-800 py-2 px-2 col-span-2 bg-[#101828] text-white
                        cursor-pointer hover:bg-gray-800'>
                                {isCustomerUpdating ? "Loading..." : "UPDATE CUSTOMER"}
                            </button>
