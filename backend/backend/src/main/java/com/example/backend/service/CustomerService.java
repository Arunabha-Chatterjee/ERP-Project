...
public class CustomerService {
    ...
    public long totalItemPurchase(String customerId) {
        Aggregation aggregation = Aggregation.newAggregation(
                Aggregation.match(Criteria.where("customerId").is(customerId)),
                ...
                Aggregation.group()
                        .sum("invoiceItemDetails.quantity").as("totalItemBuy"),

                Aggregation.project("totalItemBuy")
                        .andExclude("invoiceItemId")
        );

        AggregationResults<CustomerTotalItemPurchaseDTO> result = mongoTemplate.aggregate(
                aggregation, "invoices", CustomerTotalItemPurchaseDTO.class
        );

        CustomerTotalItemPurchaseDTO dto = result.getUniqueMappedResult();

        return dto != null ? dto.getTotalItemBuy() : 0;
    }

    public long countTotalInvoices(String customerId) {
        Query query = new Query();
        query.addCriteria(Criteria.where("customerId").is(customerId));
        return mongoTemplate.count(query, Invoice.class);
    }

    public List<String> getInvoices(String customerId) {
        Aggregation aggregation = Aggregation.newAggregation(
                Aggregation.match(Criteria.where("customerId").is(customerId)),

                Aggregation.group()
                        .addToSet("_id").as("invoices"),

                Aggregation.project("invoices")
                        .andExclude("_id")
        );

        AggregationResults<CustomerInvoicesDTO> results = mongoTemplate.aggregate(
                aggregation, "invoices", CustomerInvoicesDTO.class
        );

        CustomerInvoicesDTO dto = results.getUniqueMappedResult();

        return dto != null ? dto.getInvoices() : Collections.emptyList();
    }

    public double getTotalDue(String customerId) {
        Aggregation aggregation = Aggregation.newAggregation(
                Aggregation.match(Criteria.where("customerId").is(customerId)
                ...
                Aggregation.project("totalDue")
                        .andExclude("_id")
        );

        AggregationResults<CustomerTotalDueDTO> results = mongoTemplate.aggregate(
                aggregation, "invoices", CustomerTotalDueDTO.class
        );

        CustomerTotalDueDTO dto = results.getUniqueMappedResult();

        return dto != null ? dto.getTotalDue() : 0;
    }

    public double getTotalBilled(String customerId) {
        Aggregation aggregation = Aggregation.newAggregation(
                Aggregation.match(Criteria.where("customerId").is(customerId)),
                ...
                Aggregation.project("totalBilled")
                        .andExclude("_id")
        );
        AggregationResults<CustomerTotalBilledDTO> result = mongoTemplate.aggregate(
                aggregation, "invoices", CustomerTotalBilledDTO.class
        );

        CustomerTotalBilledDTO dto = result.getUniqueMappedResult();

        return dto != null ? dto.getTotalBilled() : 0;
    }
